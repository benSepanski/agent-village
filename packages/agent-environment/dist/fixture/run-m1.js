...
import { environmentLogs, removeEnvironment, startEnvironment, waitEnvironment, } from '../runtime.js';
...
async function main() {
    ...
    check('AC-M1.1', exitCode === 0 && report.sweep.length >= 7 && report.sweep.every((a) => a.outcome === 'failed'), `probe exit ${exitCode}; ${report.sweep.length} network attempts, outcomes: ${report.sweep
        .map((a) => a.outcome)
        .join(',')}`);
    ...
    check('AC-M1.2', report.invocations.declared_allowed.ok &&
        JSON.stringify(allowedSeq) ===
            JSON.stringify(['crossing.requested', 'crossing.decided', 'crossing.performed']) &&
        events.some((e) => e.crossing === allowedId &&
            e.event === 'crossing.decided' &&
            e.verdict === 'allow' &&
            e.decider === 'program'), `crossing ${allowedId} events: ${allowedSeq.join(' -> ')}`);
    ...
    check('AC-M1.3', !report.invocations.declared_denied.ok &&
        denied?.verdict === 'deny' &&
        denied.reason === 'payload-size-exceeded' &&
        denied.decider === 'program', `denied crossing ${deniedId}: verdict=${String(denied?.verdict)} reason=${String(denied?.reason)} decider=${String(denied?.decider)}`);
    const undeclaredId = report.invocations.undeclared.crossing;
    const undeclared = events.find((e) => e.crossing === undeclaredId && e.event === 'crossing.decided');
    check('AC-M1.4', !report.invocations.undeclared.ok &&
        undeclared?.verdict === 'deny' &&
        undeclared.reason === 'request-type-undeclared' &&
        !events.some((e) => e.crossing === undeclaredId && e.event === 'crossing.performed'), `undeclared crossing ${undeclaredId}: verdict=${String(undeclared?.verdict)} reason=${String(undeclared?.reason)}`);
    check('AC-M1.5', report.journal_paths_reachable.length === 0 &&
        JSON.stringify(report.bridge_dir) === JSON.stringify(['bridge.sock']) &&
        !report.mounts.toLowerCase().includes('journal'), `journal paths reachable in environment: [${report.journal_paths_reachable.join(',')}]; /bridge contains [${report.bridge_dir.join(',')}]`);
    const names = new Set(EVENT_NAMES);
    const envelopeOk = events.every((e) => names.has(e.event) &&
        typeof e.application_instance === 'string' &&
        typeof e.activation === 'string' &&
        typeof e.flow === 'string' &&
        typeof e.principal === 'object');
    check('AC-M1.6', envelopeOk && events.length >= 11, `${events.length} events, all names in closed set with full envelope: ${String(envelopeOk)}`);
...
//# sourceMappingURL=run-m1.js.map