...
function tcpAttempt(label, host, port) {
...
function udpAttempt(label, host, port) {
    return new Promise((done) => {
...
function dnsResolveAttempt(label, servers) {
    return new Promise((done) => {
        if (servers) {
            try {
                setServers(servers);
        ...
        const timer = setTimeout(() => done({ attempt: label, outcome: 'failed', detail: 'timeout' }), TIMEOUT_MS);
...
function dnsLookupAttempt(label) {
    return new Promise((done) => {
        const timer = setTimeout(() => done({ attempt: label, outcome: 'failed', detail: 'timeout' }), TIMEOUT_MS);
...
function findJournalPaths(root, matches, depth) {
    if (depth > 6)
        return;
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        if (['/proc', '/sys', '/dev'].includes(path))
            continue;
        if (entry.toLowerCase().includes('journal'))
            matches.push(path);
        try {
            if (statSync(path).isDirectory())
                findJournalPaths(path, matches, depth + 1);
        }
        catch {
