...
- [ADR-0004](../adr/0004-typescript-node-stack.md) (TypeScript on Node 22) written at `Proposed`
  and **accepted by the owner the same day**, in the session that built M1. The stack is settled;
  later milestones build on it without re-asking.
...
Notification sent this run (2026-08-04): ADR-0004 needs owner acceptance. The owner accepted in
the same session, so that ask is closed — do not re-send it.
