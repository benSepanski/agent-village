...
- **Google federation needs per-env setup** — the infra is wired (hosted UI domain, optional Google IdP), but each env must set `googleClientId` in config and create the client-secret in Secrets Manager before "Sign in with Google" works (see [key-properties/user-auth](../key-properties/user-auth.md)).
