...
| A config-pinned account wins over `CDK_DEFAULT_ACCOUNT` (set by the CDK CLI from the active credentials); prod pins its account from `AV_PROD_ACCOUNT_ID` (a CI repo variable, or replace with the literal id), making CDK refuse mismatched credentials. Unpinned envs fall back to whoever is deploying | [`loadEnvConfig()`](../../packages/infra/config/index.ts), [`config/prod.ts`](../../packages/infra/config/prod.ts), spread into every stack's `env` in [`bin/app.ts`](../../packages/infra/bin/app.ts) |
...
- **Pinning is opt-in.** Prod's account pin only takes effect once `AV_PROD_ACCOUNT_ID` is set (as a GitHub repo variable for CI deploys, in the shell for laptop deploys) or the literal id is written into [`config/prod.ts`](../../packages/infra/config/prod.ts). While unset, the old behavior holds: deploying `prod` with dev-account credentials creates prod-named stacks in the dev account. Dev is deliberately unpinned.
