...
| DG7 | The system stays small enough to hold in one head                               | The shipped package declares one runtime dependency (or two, if Q2 admits the reserved native slot — the allowance AC-7.4 gates on), argued in [ADR-0004](../../adr/0004-typescript-node-for-agent-cli.md); the toolkit has exactly the fifteen commands listed in Architecture; and on-disk artifacts are covered by `pnpm check` the day the first recipe lands |
...
anywhere in the request pipeline. See [ADR-0004](../../adr/0004-typescript-node-for-agent-cli.md) for
